    ...
    val displayInsets = remember {
        DisplayInsets().apply {
            ViewCompat.getRootWindowInsets(view)?.let {
                systemBars.updateFrom(it, Type.systemBars())
                systemGestures.updateFrom(it, Type.systemGestures())
                statusBars.updateFrom(it, Type.statusBars())
                navigationBars.updateFrom(it, Type.navigationBars())
                ime.updateFrom(it, Type.ime())
            }
        }
    }
...
fun Modifier.imeHeight() = composed {
    InsetsSizeModifier(
        insets = InsetsAmbient.current.ime,
        heightSide = VerticalSide.Bottom,
...
fun Modifier.imePadding() = composed {
    insetsPadding(insets = InsetsAmbient.current.ime, bottom = true)
}
